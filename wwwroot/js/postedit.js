...
const validateForm = () => {
    ...
    if (minPart < 2) errors.push({ field: "MinParticipants", message: "Min participants must be at least 2 (including you)." });
    if (maxPart < 2) errors.push({ field: "MaxParticipants", message: "Max participants must be at least 2 (including you)." });
    if (minPart >= 2 && maxPart >= 2 && minPart > maxPart) errors.push({ field: "MinParticipants", message: "Min participants cannot be greater than max participants." });
...
const form = document.querySelector('form[action*="PostEdit"]');
if (form) {
...
document.getElementById("submit-btn").addEventListener('click', async (e) => {
    ...
    if (imgKeyInput && imageResult.fileName) {
...
let initialLat = document.getElementById("location-lat")?.value || 13.7563;
let initialLon = document.getElementById("location-lon")?.value || 100.5018;
...
if (document.getElementById("location-lat")?.value && document.getElementById("location-lon")?.value) {
    marker = L.marker([initialLat, initialLon]).addTo(map);
    let locName = document.getElementById("LocationName")?.value || "Selected Location";
    marker.bindPopup("<b>Place :</b><br>" + locName).openPopup();
}
